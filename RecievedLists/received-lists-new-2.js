    ...
    try {
        ...
        try {
            if (newEmailRows.length > 0) {
                ...
                //currentList.getRange(1, emailColumn + 1).setBackground("#f5bfb3");
...
/* function GetRejRanges(curSheet,lastRow, rejColumns, rejColumn, rejColumn){
    if (!rejColumns[rejName + 'BgColors'] ){
        rejColumns[rejName + 'BgColors'] = curSheet.getRange(2,rejColumn,lastRow).getBackgrounds();
    }
} */
...
function getRejectionRate(result, curSheet, sheetName, date, link) {
    try {
        ...
        try {
            ...
            var titleColumn = result.ColumnNames['title'] + 1
            var countryColumn = result.ColumnNames['country'] + 1
            var employeesColumn = result.ColumnNames['employees'] + 1
            var revenueColumn = result.ColumnNames['revenue'] + 1
            var industryColumn = result.ColumnNames['industry'] + 1
            var companyColumn = result.ColumnNames['company'] + 1
            ...
            /* Browser.msgBox("title "+ typeof  titleColumn + "   " + titleColumn)
            Browser.msgBox("title "+ typeof  titleColumn2 + "   " + titleColumn2)
            Browser.msgBox("country "+ typeof  countryColumn + "   " + countryColumn)
            Browser.msgBox("country "+ typeof  countryColumn2 + "   " + countryColumn2)
            Browser.msgBox("employeesColumn "+ typeof  employeesColumn + "   " + employeesColumn)
            Browser.msgBox("employeesColumn "+ typeof  employeesColumn2 + "   " + employeesColumn2)
            Browser.msgBox("revenueColumn "+ typeof  revenueColumn + "   " + revenueColumn)
            Browser.msgBox("revenueColumn "+ typeof  revenueColumn2 + "   " + revenueColumn2)
            Browser.msgBox("industryColumn "+ typeof  industryColumn + "   " + industryColumn)
            Browser.msgBox("industryColumn "+ typeof  industryColumn2 + "   " + industryColumn2)
            Browser.msgBox("companyColumn "+ typeof  companyColumn + "   " + companyColumn)
            Browser.msgBox("companyColumn "+ typeof  companyColumn2 + "   " + companyColumn2) */

        } catch (err) {
            Browser.msgBox("err in 6.00" + err)
        }
        // var currentTime = (new Date).getTime();
        //T.time += (currentTime - startTime);
        //Browser.msgBox(T.time)
        var lastRow = result.LastRow;


        //var OV_comments = curSheet.getRange(2,OV_CommentColumn,curSheet.getLastRow()).getValues();
        var countChecked = getCheckedLeads(curSheet, lastRow, result.ColumnNames['ov_comment']);
        ...
        var countsGreenAndOthers = getRejCounts(curSheet, titleColumn, lastRow);
        ...
        countsGreenAndOthers = getRejCounts(curSheet, countryColumn, lastRow);
        ...
        countsGreenAndOthers = getRejCounts(curSheet, industryColumn, lastRow);
        ...
        countsGreenAndOthers = getRejCounts(curSheet, employeesColumn, lastRow);
        ...
        countsGreenAndOthers = getRejCounts(curSheet, revenueColumn, lastRow);
        ...
        var rejNac = getRejNac(curSheet, companyColumn, result);
...
function setBackground2(rejSheet, dateScriptRun, lastCellDate) {
...
function getRejNac(curSheet, columnIndex, result) {

    var countRejection = 0;
    if (columnIndex <= 0) return 0;
    var columns = curSheet.getRange(1, 1, 1, result.LastColumn).getValues();
    var OV_CommentColumn = result.ColumnNames['ov_comment']

    var OV_Comments = curSheet.getRange(2, OV_CommentColumn, result.LastRow).getValues();
    try {
        for (var i = 0; i < OV_Comments.length; i++) {
            if (OV_Comments[i].toString().indexOf('NAC') != -1 || OV_Comments[i].toString().indexOf('SUP') != -1)
                countRejection++;
        }
    } catch (err) { Browser.msgBox("New error 7.0" + err) }
    return countRejection;
}
function getRejCounts(curSheet, columnIndex, lastRow) {
    var countRejectionGreen = 0, countRejectionYellow = 0;
    if (columnIndex <= 0) return 0;
    var rangeColumn = curSheet.getRange(2, columnIndex, lastRow);
